...
<script>
	$(function(){
		$("#arrayKey").on("change", function(){
			location.href="<%=projectPath%>/campaign/ex_Campaign.do?arrayKey="+$(this).val();
		});
	});
</script>
<body>
		...
		<div id="total" class="w-full max-w-screen-xl my-0 mx-auto flex flex-wrap mb-12">
			<c:forEach var="vo" items="${list}">
				<div class=" w-1/3 px-4 mb-12">
					<a href="<%=projectPath%>/campaign/detail_Campaign.do">
			...
			<div class="w-full max-w-screen-xl my-0 mx-auto mb-12">
				<ul class="flex justify-center" id="page">
					...
					<c:if test="${pageVO.pageNum!=1}">
						<li class="hover:text-danger"><a href="<%=projectPath%>/campaign/ex_Campaign.do?pageNum=${pageVO.pageNum-1}<c:if test='${pageVO.arrayKey!=null}'>&arrayKey=${pageVO.arrayKey}</c:if>"><i class="xi-angle-left"></i></a></li>
					</c:if>
					<c:forEach var="i" begin="${pageVO.startPage}" end="${pageVO.startPage + pageVO.onePageCount-1}">
						<c:if test="${i<=pageVO.totalPage}">
							<li class="hover:text-danger"><a href="<%=projectPath%>/campaign/ex_Campaign.do?pageNum=${i}<c:if test='${pageVO.arrayKey!=null}'>&arrayKey=${pageVO.arrayKey}</c:if>" <c:if test='${i==pageVO.pageNum}'>style='color:red'</c:if>>${i}</a></li>
						</c:if>
					</c:forEach>
					...
					<c:if test="${pageVO.pageNum < pageVO.totalPage}">
						<li class="hover:text-danger"><a href="<%=projectPath%>/campaign/ex_Campaign.do?pageNum=${pageVO.pageNum+1}<c:if test='${pageVO.arrayKey!=null}'>&arrayKey=${pageVO.arrayKey}</c:if>"><i class="xi-angle-right"></i></a></li>
					</c:if>
				</ul>
			</div>
