			...
			<div class="card_box w-full my-0 mx-auto max-w-screen-xl"> 
				...
				<nav id="notice-navigation" class="flex items-center justify-between flex-wrap flex-grow w-3/4 my-0 mx-auto pt-8">
						<ul class="notice-menu flex flex-wrap flex-grow block text-lg"> 
							...
							<li id="tab_03" class="notice_menu_item rounded-lg bg-danger-dark text-white text-center flex items-center mr-2"><a href="<%=projectPath %>/news/article.do" class="  ">보도자료</a></li> 
						</ul> 					    
									...
									<div class="border-b border-l border-gray-lighte lg:border-l-0 lg:border-gray-lighte bg-white px-4 py-8 flex justify-between leading-normal">
					  					<div class="w-2/3">  
					    					<div class="mb-8"> 
					      						...
					      						<div class="text-gray-darkest font-bold text-xl mb-2"><a href="<%=projectPath %>/news/newsContent.do?news_no=${vo.news_no}&pageNum=${pVo.pageNum}&news_category=${vo.news_category}">${vo.news_title}</a></div>
					      	  					<p class="text-gray-dark text-base">  
					      	  						<a href="<%=projectPath %>/news/newsContent.do?news_no=${vo.news_no}&pageNum=${pVo.pageNum}&news_category=${vo.news_category}">${vo.news_content}</a> 
					      	  					</p>
					    	  		 		</div> 
					   						<div>   
						...
						<div class="pageDiv">
							...
							<ul id="main_QnA_page" class="pagenation flex items-center justify-center my-4">
								...
								<li class="page-item"> 
									...
									<c:if test="${pVo.pageNum > 1 }">
										<a class="page-link block py-1 px-2 hover:text-danger" href="<%=projectPath%>/news/article.do?pageNum=${pVo.pageNum - 1}"><i class="xi-angle-left-min"></i></a>
									</c:if>	
								</li>
								<!-- 페이지 수 -->
								<c:forEach var="i" begin="${pVo.startPage }" end="${pVo.startPage + pVo.pageCount - 1 }">
									<c:if test="${i <= pVo.totalPage }">  
										<li class="page-item acitve"><a class="pn page-link block py-1 px-2 hover:text-brand text-black" href="<%=projectPath%>/news/article.do?pageNum=${i}"<c:if test="${i == pVo.pageNum }">style='color:red'</c:if>>${i}</a></li>
									</c:if>
								</c:forEach>	
								<!-- next -->
								<li class="page-item">
									...
									<c:if test="${pVo.pageNum < pVo.totalPage}">  
										<a class="page-link block py-1 px-2 hover:text-danger" href="<%=projectPath%>/news/article.do?pageNum=${pVo.pageNum + 1}"><i class="xi-angle-right-min"></i></a>
									</c:if>		
								</li>
							</ul>	
						</div>				
