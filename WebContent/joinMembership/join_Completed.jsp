<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>join_Completed</title>
</head>
<body>

			<div class="w-full max-w-screen-xl my-0 mx-auto justify-center">
				<div class="text-6xl text-center mt-24">환영합니다!</div>
				<div class="text-center my-10">회원가입을 축하합니다!<br><span class="text-danger">ChariTree</span>에서는 항상 회원님들의 입장에서<br>
				보다 좋은 서비스를 받으실 수 있도록 노력하겠습니다.<br>감사합니다 :D</div>
			</div>
			<a href="<%=projectPath%>/index.do"><div class="w-24 h-10 border my-5 mx-auto text-center rounded">
					<div class="justify-center leading-9 home h-full w-full">홈으로</div>
				</div></a>

</body>
</html>